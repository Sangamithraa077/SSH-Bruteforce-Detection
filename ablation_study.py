...
def evaluate_ablation_balancing():
    print("==================================================")
    print("Evaluating Balancing on Behavioral Features Only (Ablation Study)")
    print("==================================================")
    ...
    # We want to exclude explicit authentication result features:
    # 'failed_count', 'success_count', 'invalid_user_count', 'failure_rate', 'invalid_user_rate', 'success_rate'
    # Keeping only:
    ...
    print("Behavioral feature indices:", behavioral_indices)
    
    # Load test sets
    raw_data = np.load(os.path.join(PROCESSED_DIR, "split_raw.npz"))
    X_val = raw_data['X_val'][:, behavioral_indices]
    y_val = raw_data['y_val']
    X_test = raw_data['X_test'][:, behavioral_indices]
    y_test = raw_data['y_test']
    
    dataset_b = np.load(os.path.join(PROCESSED_DIR, "dataset_b.npz"))
    X_b = dataset_b['X'][:, behavioral_indices]
    y_b = dataset_b['y']
    ...
    for config_name, path in configs.items():
        print(f"Training on: {config_name}...")
        data = np.load(path)
        X_train = data['X_train'][:, behavioral_indices]
    ...
    df_results.to_csv(os.path.join(PROCESSED_DIR, "behavioral_balancing_evaluation_results.csv"), index=False)
