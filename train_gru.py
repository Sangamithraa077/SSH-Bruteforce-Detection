"""
train_gru.py  —  GRU model for SSH Brute Force Detection
Architecture : Sequence input → GRU(64) → Dropout(0.3)
               → GRU(32) → Dropout(0.3) → Dense(1, sigmoid)
GRU is faster than LSTM (fewer gates) and often performs comparably.
"""
...
def train_gru():
    print("=" * 50)
    print("Training Model 3: GRU Classifier")
    ...
    model     = GRUClassifier(INPUT_SIZE, HIDDEN_SIZE, HIDDEN_SIZE2).to(DEVICE)
    ...
    path_pt   = os.path.join(MODELS_DIR, "gru_model.pt")
    path_meta = os.path.join(MODELS_DIR, "gru_meta.joblib")
